// Here is the code that will run the harpsichord instrument of the synthesizer
...
void sample_create(struct sample_create_s *, int, int);
...
// define general Variables --- 
	...
	int sc_flag = 0;					// flag for active notes
...
// Initialize stuff --- 
	...
	int rand_ptr = open("/dev/urandom",O_RDONLY);	// random number generator
	if(rand_ptr == 0){
		fprintf(stderr,"Coudln't open the random number genrator\n");
		exit(1);
	}
			...
			if((0xFF00 & input_flags[1]) != 0){ // Key press
				if(note_number >= 0 && note_number < 71){
					...
					if(i==8){
						...
						printf("not enough channels - Wrapping to use oldest!!!Y!!!!\n");
					} else {
		...
		for(i=0;i<CHANNELS;i++){		
			if(samp_create[i].active_flag == 1){
				if(samp_create[i].cycle_count == 0){
					...
					sample_create(samp_create, i, rand_ptr);  // create a period of samples;
...
void sample_create(struct sample_create_s *sc, int current, int rand_ptr){
	int i,j,k;
	double vinh = 0, v = 0, L = 0, in, in_old;
	int rand;
	double amp = 1.0; // amplitude of plucked excitation
	double len; // length of plucked excitation

	if(sc[current].note.string.fs == 22050){
		 len = 0.1;
		 amp = 0.8;
	}
	if(sc[current].note.string.fs == 44100){
		len = 0.2;
		amp = 1.1;
	}
	if(sc[current].note.string.fs == 88200){
		len = 0.8;
		amp = 2.0;
	}
	...
	for(k=0;k<FRAMES*sc[current].f_sc;k++){ // Generate 1 full period
		...
		if(DURATION*SAMPLING_FREQ/FRAMES - sc[current].cycle_count < sc[current].note.string.Mup * len){
			read(rand_ptr, &rand, sizeof(int));
			if(rand > 0){
				L = amp;
			} else {
				L = -amp;
...
int soundcard_output(snd_pcm_t * handle, double * output){
	int sample;
	int amp = 30000;	// TODO: Change it to volume control
	int i;
	...
	if(f_ptr == NULL){
		...
		fprintf(stderr, "Using default full blast\n");
		volume = 1;
	} else {
		fscanf(f_ptr, "%s",c);
		fclose(f_ptr);
		...
		if(mididata[0] == 0xFE){
			...
			switch(mididata[0]){
				...
				case 130: // Harpsichord
					...
					break;
				case 131: // Clarinnet
					printf("User Seleected Clarinnet\n");
					proc_msg_ptr = fopen("proc_msg","w");
					if(proc_msg_ptr != 0){
						usleep(10000);
						close(midi);
