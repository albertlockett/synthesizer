...
void sample_create(struct sample_create_s *sc, int current, int rand_ptr){
	...
	int rand;
	double amp = 0.8; // amplitude of plucked excitation
	double len; // length of plucked excitation

	if(sc[current].note.string.fs == 22050) len = 0.1;
	if(sc[current].note.string.fs == 44100) len = 0.2;
	if(sc[current].note.string.fs == 88200) len = 0.8;
